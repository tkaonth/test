        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-5">
                <div class="flex justify-between items-center my-2 w-full">
                    ...
                    <div class="flex grow">
                        <form class="flex" action="{{route('searchbranch')}}" method="post">
                            @csrf
                    ...
                    <div>
                        @if (Route::has('newbranchform'))
                            <a href="{{ route('newbranchform') }}"
                                class="bg-blue-500 text-white drop-shadow-lg p-2 w-full rounded-lg hover:bg-blue-300">{{ $content_lang['add-branch'] }}</a>
                        @endif
                    </div>
                </div>
                ...
                <table class="w-full table mb-2 table-auto border-2 text-center">
                    <thead class="bg-gray-400 drop-shadow-lg text-lg">
                        <tr>
                            ...
                            <th class="px-4 py-2">{{ $content_lang['table-zone'] }}</th>
                            ...
                            @foreach ($branches as $branch)
                                ...
                                <tr class="border-2 hover:bg-gray-300">
                                    ...
                                    @elseif(session()->get('locale') == 'en')
                                        <td>{{ $branch->eng }}</td>
                                    @endif
                                    <td class="max-w-md p-3">{{ $branch->zone }}</td>
                                    <td>
                                        <a href="{{ route('editbranchform', ['id' => $branch->id]) }}">
                                            <div
                                    ...
                                    <td>
                                        ...
                                        <form id="delete-user{{ $branch->id }}" class="w-full"
                                            action="{{ route('deletebranch', ['id' => $branch->id]) }}"
                                            method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                            <button onclick="confirmSubmit('delete-user{{ $branch->id }}')"
                                                type="button"
