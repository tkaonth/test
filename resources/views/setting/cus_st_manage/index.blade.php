@php
    ...
    $content_lang = include base_path('lang\\' . session()->get('locale') . '\cus_st.php');
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-5">
                <div class="flex justify-between items-center my-2 w-full">
                    ...
                    <div class="flex grow">
                        <form class="flex" action="{{ route('searchcus-st') }}" method="post">
                            @csrf
                    ...
                    <div>
                        @if (Route::has('newcus-st'))
                            <a href="{{ route('newcus-st') }}"
                        ...
                        @else
                            @foreach ($cus_sts as $cus_st)
                                ...
                                <tr class="border-2 hover:bg-gray-300">
                                    ...
                                    @endif
                                    <td>
                                        <a href="{{ route('editcus-st', ['id' => $cus_st->id]) }}">
                                            <div
                                    ...
                                    <td>
                                        <form id="delete-cus-st{{ $cus_st->id }}" class="w-full"
                                            action="{{ route('deletecus-st', ['id' => $cus_st->id]) }}"
                                            method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                            <button onclick="confirmSubmit('delete-cus-st{{ $cus_st->id }}')"
                                                type="button"
