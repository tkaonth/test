        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg p-5">
                <div class="flex justify-between items-center my-2 w-full">
                    ...
                    <div class="flex grow">
                        <form class="flex" action="{{ route('searchdoc-st') }}" method="post">
                            @csrf
                    ...
                    <div>
                        @if (Route::has('newdoc-st'))
                            <a href="{{ route('newdoc-st') }}"
                        ...
                        @else
                            @foreach ($doc_sts as $doc_st)
                                ...
                                <tr class="border-2 hover:bg-gray-300">
                                    ...
                                    @endif
                                    <td>
                                        <a href="{{ route('editdoc-st', ['id' => $doc_st->id]) }}">
                                            <div
                                    ...
                                    <td>
                                        <form id="delete-doc-st{{ $doc_st->id }}" class="w-full"
                                            action="{{ route('deletedoc-st', ['id' => $doc_st->id]) }}"
                                            method="POST" class="d-inline">
                                            @csrf
                                            @method('DELETE')
                                            <button onclick="confirmSubmit('delete-doc-st{{ $doc_st->id }}')"
                                                type="button"
