                        ...
                        <div class="rounded-lg drop-shadow-lg">
                            <ul class="bg-gray-50 rounded-lg p-4">
                                @foreach ($doc_lists as $doc_list)
                                    @php
                                        $fileext = pathinfo($doc_list->bill_filename, PATHINFO_EXTENSION);
                                        
                                    @endphp
                                        <li class="flex items-center p-2 rounded-lg hover:bg-gray-200">
                                            
                                            <img @if($fileext == 'pdf') src="{{ asset('/getuploadicon/document_icon.png') }}" @else src="{{ asset($doc_list->bill_doc_path . '/' . $doc_list->bill_filename) }}" @endif
                                                alt="{{$doc_list->bill_filename}}" width="50" height="auto">
                                            <div class="ml-3 flex justify-between items-center w-full">
                                                <div class="text-blue-500">
                                                    {{ $doc_list->bill_name }}
                                                </div>
                                                <div class="flex space-x-2">
                                                    <a target="_blank" href="{{ asset($doc_list->bill_doc_path . '/' . $doc_list->bill_filename) }}">
...
<script>
    ...
    function confirmremovefile(e) {
        console.log(e);
        var url = "{{ route('removebill', ':id') }}".replace(':id', e);
        Swal.fire({
